...
class Solution:   
    def isSubtree(self, root: Optional[TreeNode], subRoot: Optional[TreeNode]) -> bool:
        ...
        if self.same_tree(root,subRoot): #In case root has different val but subtree root has same val and is a subtree
            return True
        # We are checking if above condition root val are not then lets check if subtree is present in left side/nodes of root
        # OR subtree is present in root right side nodes. If present or True in one case, then we found and Return true
